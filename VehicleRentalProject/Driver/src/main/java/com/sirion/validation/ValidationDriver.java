...
public class ValidationDriver  {
	...
	public boolean DriverValidation(Driver d) throws DriverException {
		...
		if(d.getContactNumber().length() != 10 || !d.getContactNumber().matches("\\d+")) {
			throw new DriverException("Phone number is not valid");
		}
	     ...
	     if(d.getFirstName().isBlank() || d.getLastName().isBlank() || d.getLicenseNo().isBlank()) {
	    	 throw new DriverException("First Name ,Last Name or License is not valid");
	     }
